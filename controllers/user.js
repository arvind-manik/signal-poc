...
exports.getLoginInfo = (req, res) => {
  if (req.user) {
    res.send(JSON.stringify(sessionUtils.getWritableUser(req.user)));
    return;
  }
...
/**
 * Get all live users for data
 */

exports.getUsers = (req, res) => {
  const clients = sessionUtils.getLiveUsers();
  const conenctedIds = [];
  clients.forEach((client) => {
    conenctedIds.push(client.user_id);
  });

  const usersList = [];
  User.find().where('_id').in(conenctedIds).exec((error, records) => {
    records.forEach((user) => {
      usersList.push(sessionUtils.getWritableUser(user));
    });
  });
...
exports.getUser = (req, res) => {
  ...
  User.findById(userId, (err, user) => {
    if (user) {
      res.send(JSON.stringify(sessionUtils.getWritableUser(user)));
    } else if (err != null) {
      throw err;
    }
  });
...
exports.getProfilePic = (req, res) => {
  ...
  User.findById(userId, (err, user) => {
    if (user) {
      const image = user.profile && user.profile.picture ? user.profile.picture : user.gravatar(60);
      res.send(image);
    } else if (err != null) {
      throw err;
    }
