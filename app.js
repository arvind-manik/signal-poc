...
const socketsGen = require('socket.io');
...
const keysControlller = require('./controllers/keys');
...
app.set('port', parseInt(process.env.PORT || process.env.OPENSHIFT_NODEJS_PORT || 8080, 10));
app.set('wsport', parseInt(process.env.WSPORT || 9191, 10));
app.set('views', path.join(__dirname, 'views'));
app.set('view engine', 'pug');

const io = socketsGen(app.get('wsport'));
app.use(expressStatusMonitor({ websocket: io, port: app.get('wsport') }));
...
encryptor.initialize(process.env.AES_KEY);
const cookieMiddleware = cookieHandler.initialize(encryptor);
...
app.get('/me', userController.getLoginInfo);
app.get('/users', userController.getUsers);
app.get('/users/:user_id', userController.getUser);
app.get('/users/:user_id/picture', userController.getProfilePic);
...
app.post('/keys/:user_id', keysControlller.shareKey);
...
require('./utils/sockets').initialize(io);
